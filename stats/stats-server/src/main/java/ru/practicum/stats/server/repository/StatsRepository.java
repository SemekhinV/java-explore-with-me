...
public interface StatsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query(value = "SELECT NEW ru.practicum.stats.server.entity.ViewStats(hit.uri, hit.app, COUNT(hit.ip)) "
            + "FROM Hit hit "
    ...
    @Query(value = "SELECT NEW ru.practicum.stats.server.entity.ViewStats(hit.uri, hit.app, COUNT(DISTINCT hit.ip)) "
            + "FROM Hit hit "
            + "WHERE hit.uri IN ?3 AND hit.timestamp BETWEEN ?1 AND ?2 "
            + "GROUP BY hit.uri, hit.app "
